...
# from box_utils import match, log_sum_exp, decode, center_size, crop, elemwise_box_iou, sanitize_coordinates
import box_utils
# from config import config.cfg, mask_type, activation_func
import config
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio):
        ...
        if config.cfg.use_class_balanced_conf:
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        if config.cfg.mask_type == config.mask_type.lincomb:
            proto_data = predictions['proto']

        score_data = predictions['score'] if config.cfg.use_mask_scoring   else None   
        inst_data  = predictions['inst']  if config.cfg.use_instance_coeff else None
        ...
        # if config.cfg.mask_type == mask_type.lincomb:
        ...
        for idx in range(batch_size):
            ...
            # print("---- targets[idx]", targets[idx][:, :-1])
            ...
            box_utils.match(self.pos_threshold, self.neg_threshold,
        ...
        if config.cfg.train_boxes:
            loc_p = loc_data[pos_idx].view(-1, 4)
            loc_t = loc_t[pos_idx].view(-1, 4)
            losses['B'] = F.smooth_l1_loss(loc_p, loc_t, reduction='sum') * config.cfg.bbox_alpha

        if config.cfg.train_masks:
            if config.cfg.mask_type == config.mask_type.direct:
                if config.cfg.use_gt_bboxes:
                    pos_masks = []
                    for idx in range(batch_size):
                        pos_masks.append(masks[idx][idx_t[idx, pos[idx]]])
                    masks_t = torch.cat(pos_masks, 0)
                    masks_p = mask_data[pos, :].view(-1, config.cfg.mask_dim)
                    losses['M'] = F.binary_cross_entropy(torch.clamp(masks_p, 0, 1), masks_t, reduction='sum') * config.cfg.mask_alpha
                else:
                ...
                if config.cfg.mask_proto_loss is not None:
                    if config.cfg.mask_proto_loss == 'l1':
                        losses['P'] = torch.mean(torch.abs(proto_data)) / self.l1_expected_area * self.l1_alpha
                    elif config.cfg.mask_proto_loss == 'disj':
        ...
        # Mask IoU Loss
        if config.cfg.use_maskiou and maskiou_targets is not None:
    ...
    def class_existence_loss(self, class_data, class_existence_t):
        return config.cfg.class_existence_alpha * F.binary_cross_entropy_with_logits(class_data, class_existence_t, reduction='sum')

    def semantic_segmentation_loss(self, segment_data, mask_t, class_t, interpolation_mode='bilinear'):
        # Note num_classes here is without the background class so config.cfg.num_classes-1
        ...
        return loss_s / mask_h / mask_w * config.cfg.semantic_segmentation_alpha


    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        # Compute max conf across batch for hard negative mining
        batch_conf = conf_data.view(-1, self.num_classes)
        if config.cfg.ohem_use_most_confident:
            ...
            loss_c = box_utils.log_sum_exp(batch_conf) - batch_conf[:, 0]
        ...
        return config.cfg.conf_alpha * loss_c
    ...
    def focal_conf_loss(self, conf_data, conf_t):
        ...
        at = (1 - config.cfg.focal_loss_alpha) * background + config.cfg.focal_loss_alpha * (1 - background)

        loss = -at * (1 - pt) ** config.cfg.focal_loss_gamma * logpt

        # See comment above for keep
        return config.cfg.conf_alpha * (loss * keep).sum()
    ...
    def focal_conf_sigmoid_loss(self, conf_data, conf_t):
        ...
        at = config.cfg.focal_loss_alpha * conf_one_t + (1 - config.cfg.focal_loss_alpha) * (1 - conf_one_t)
        at[..., 0] = 0 # Set alpha for the background class to 0 because sigmoid focal loss doesn't use it

        loss = -at * (1 - pt) ** config.cfg.focal_loss_gamma * logpt
        loss = keep * loss.sum(dim=-1)

        return config.cfg.conf_alpha * loss.sum()
    ...
    def focal_conf_objectness_loss(self, conf_data, conf_t):
        ...
        obj_loss = -at * (1 - pt) ** config.cfg.focal_loss_gamma * logpt
        ...
        return config.cfg.conf_alpha * (class_loss + (obj_loss * keep).sum())
    ...
    def conf_objectness_loss(self, conf_data, conf_t, batch_size, loc_p, loc_t, priors):
        ...
        with torch.no_grad():
            ...
            boxes_pred = box_utils.decode(loc_p, pos_priors, config.cfg.use_yolo_regressors)
            boxes_targ = box_utils.decode(loc_t, pos_priors, config.cfg.use_yolo_regressors)

            iou_targets = box_utils.elemwise_box_iou(boxes_pred, boxes_targ)
        ...
        return config.cfg.conf_alpha * (class_loss + obj_pos_loss + obj_neg_loss)
    ...
    def direct_mask_loss(self, pos_idx, idx_t, loc_data, mask_data, priors, masks):
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                pos_bboxes = box_utils.decode(loc_data[idx, :, :], priors.data, config.cfg.use_yolo_regressors)
                ...
                x1, x2 = box_utils.sanitize_coordinates(pos_bboxes[:, 0], pos_bboxes[:, 2], img_width)
                y1, y2 = box_utils.sanitize_coordinates(pos_bboxes[:, 1], pos_bboxes[:, 3], img_height)
                ...
                for jdx in range(num_pos):
                    ...
                    new_mask = F.adaptive_avg_pool2d(tmp_mask.unsqueeze(0), config.cfg.mask_size)
            ...
            loss_m += F.binary_cross_entropy(torch.clamp(pos_mask_data, 0, 1), mask_t, reduction='sum') * config.cfg.mask_alpha
    ...
    def coeff_diversity_loss(self, coeffs, instance_t):
        ...
        return config.cfg.mask_proto_coeff_diversity_alpha * loss.sum() / num_pos
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, interpolation_mode='bilinear'):
        ...
        process_gt_bboxes = config.cfg.mask_proto_normalize_emulate_roi_pooling or config.cfg.mask_proto_crop

        if config.cfg.mask_proto_remove_empty_masks:
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                # print("---- masks[idx].unsqueeze(0).dtype", masks[idx].unsqueeze(0).dtype)
                # print("---- masks[idx].unsqueeze(0)", masks[idx].unsqueeze(0))
                
                downsampled_masks = F.interpolate(masks[idx].unsqueeze(0), (mask_h, mask_w),
                                                  mode=interpolation_mode, align_corners=False).squeeze(0)
                downsampled_masks = downsampled_masks.permute(1, 2, 0).contiguous()

                if config.cfg.mask_proto_binarize_downsampled_gt:
                    downsampled_masks = downsampled_masks.gt(0.5).float()
                ...
                if config.cfg.mask_proto_reweight_mask_loss:
                    # Ensure that the gt is binary
                    if not config.cfg.mask_proto_binarize_downsampled_gt:
                    ...
                    mask_reweighting   = gt_foreground_norm * config.cfg.mask_proto_reweight_coeff + gt_background_norm
            ...
            if process_gt_bboxes:
                ...
                if config.cfg.mask_proto_crop_with_pred_box:
                    pos_gt_box_t = box_utils.decode(loc_data[idx, :, :], priors.data, config.cfg.use_yolo_regressors)[cur_pos]
                else:
            ...
            if config.cfg.mask_proto_coeff_diversity_loss:
                if inst_data is not None:
            ...
            if old_num_pos > config.cfg.masks_to_train:
                perm = torch.randperm(proto_coef.size(0))
                select = perm[:config.cfg.masks_to_train]
            ...
            pred_masks = config.cfg.mask_proto_mask_activation(pred_masks)

            if config.cfg.mask_proto_double_loss:
                ...
                loss_m += config.cfg.mask_proto_double_loss_alpha * pre_loss

            if config.cfg.mask_proto_crop:
                pred_masks = box_utils.crop(pred_masks, pos_gt_box_t)
            ...
            if config.cfg.mask_proto_normalize_mask_loss_by_sqrt_area:
            ...
            if config.cfg.mask_proto_normalize_emulate_roi_pooling:
                weight = mask_h * mask_w if config.cfg.mask_proto_crop else 1
                pos_gt_csize = box_utils.center_size(pos_gt_box_t)
            ...
            if config.cfg.use_maskiou:
                if config.cfg.discard_mask_area > 0:
                    gt_mask_area = torch.sum(mask_t, dim=(0, 1))
                    select = gt_mask_area > config.cfg.discard_mask_area
        ...
        losses = {'M': loss_m * config.cfg.mask_alpha / mask_h / mask_w}
        ...
        if config.cfg.use_maskiou:
            ...
            if config.cfg.maskious_to_train > 0 and num_samples > config.cfg.maskious_to_train:
    ...
    def mask_iou_loss(self, net, maskiou_targets):
        ...
        return loss_i * config.cfg.maskiou_alpha
