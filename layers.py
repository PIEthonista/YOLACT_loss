...
# from box_utils import box_utils.decode, box_utils.jaccard
import box_utils
...
class Detect(object):
    """At test time, Detect is the final layer of SSD.  Box_utils.decode location preds,
    ...
    def __call__(self, predictions, net):
        ...
        with utils.timer.env('Detect'):
            ...
            for batch_idx in range(batch_size):
                box_utils.decoded_boxes = box_utils.decode(loc_data[batch_idx], prior_data)
                result = self.detect(batch_idx, conf_preds, box_utils.decoded_boxes, mask_data, inst_data)
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        boxes = box_utils.decoded_boxes[keep, :]
    ...
    def cc_fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200):
        ...
        iou = box_utils.jaccard(boxes_idx, boxes_idx)
    ...
    def fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200, second_threshold:bool=False):
        ...
        iou = box_utils.jaccard(boxes, boxes)
        iou.triu_(diagonal=1)
        ...
        # Only keep the top config.cfg.max_num_detections highest scores across all classes
        scores, idx = scores.sort(0, descending=True)
        idx = idx[:config.cfg.max_num_detections]
        scores = scores[:config.cfg.max_num_detections]
    ...
    def traditional_nms(self, boxes, masks, scores, iou_threshold=0.5, conf_thresh=0.05):
        ...
        boxes = boxes * config.cfg.max_size
        ...
        idx2 = idx2[:config.cfg.max_num_detections]
        scores = scores[:config.cfg.max_num_detections]
        ...
        return boxes[idx] / config.cfg.max_size, masks[idx], classes, scores
