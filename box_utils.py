...
def match(pos_thresh, neg_thresh, truths, priors, labels, crowd_boxes, loc_t, conf_t, idx_t, idx, loc_data):
    ...
    """
    decoded_priors = decode(loc_data, priors, config.cfg.use_yolo_regressors) if config.cfg.use_prediction_matching else point_form(priors)
    
    # Size [num_objects, num_priors]
    overlaps = jaccard(truths, decoded_priors) if not config.cfg.use_change_matching else change(truths, decoded_priors)
    ...
    if crowd_boxes is not None and config.cfg.crowd_iou_threshold < 1:
        # Size [num_priors, num_crowds]
        crowd_overlaps = jaccard(decoded_priors, crowd_boxes, iscrowd=True)
        # Size [num_priors]
        best_crowd_overlap, best_crowd_idx = crowd_overlaps.max(1)
        # Set non-positives with crowd iou of over the threshold to be neutral.
        conf[(conf <= 0) & (best_crowd_overlap > config.cfg.crowd_iou_threshold)] = -1

    loc = encode(matches, priors, config.cfg.use_yolo_regressors)
